...
public class ItemThreatTracker : AIModule
{
    ...
    {
        public ItemThreatTracker owner;
        public ThreatPoint threatPoint;
        ...
        public ThreatItem(ItemThreatTracker owner, ItemTracker.ItemRepresentation itemRep)
        {
            ...
            severity = owner.AI.DynamicItemRelationship(itemRep.representedItem).intensity / 2f;
            if (owner.AI.DynamicItemRelationship(itemRep.representedItem).type == CreatureTemplate.Relationship.Type.Afraid)
            {
                severity += 0.5f;
            }
    ...
    public List<ThreatPoint> threatPoints;
    public List<ThreatItem> threatItems;
    ...
    public virtual float Panic => 1 - 1 / (currentThreatLevel + 1f);
    ...
    public override float Utility()
    {
        ...
        return Mathf.Clamp(Mathf.Lerp(-0.01f, 1.01f, currentThreatLevel), 0, 1);
    }
    ...
    public override void Update()
    {
        ...
        if (resetCounter < 200)
        {
            ...
            {
                currentThreatLevel = Mathf.Pow(currentThreatLevel, 1 / (1 + threatToBeat * 3f));
            }
        }
    ...
    public WorldCoordinate FleeTo(WorldCoordinate occupyTile, int maxReevalutaions, int maximumDistance, bool considerLeavingRoom, bool considerGoingHome = false)
    {
    ...
    public virtual List<IntVector2> GenerateRandomPath(WorldCoordinate occupyTile, int length)
    {
        if (!AI.pathFinder.CoordinateViable(occupyTile))
        {
            return null;
    ...
    public virtual float EvaluateFlightDestThreat(WorldCoordinate occupyTile, WorldCoordinate coord, int maximumDistance, ref List<IntVector2> scratchPath)
    {
        if (coord.room != occupyTile.room ||
            Custom.ManhattanDistance(coord, occupyTile) >= maximumDistance * 2 ||
            !AI.pathFinder.CoordinateViable(coord))
        {
            return float.MaxValue;
        }

        int tileCount = AI.creature.Room.realizedRoom.RayTraceTilesList(occupyTile.x, occupyTile.y, coord.x, coord.y, ref scratchPath);
        bool PathFullyAccessible = true;
        for (int i = 0; i < tileCount && PathFullyAccessible; i++)
        {
            if (!aiMap.TileAccessibleToCreature(scratchPath[i], AI.creature.creatureTemplate))
            {
                PathFullyAccessible = false;
            }
        }

        if (PathFullyAccessible)
        {
            return ThreatOfPath(scratchPath, tileCount);
        }

        tileCount = QuickConnectivity.QuickPath(AI.creature.Room.realizedRoom, AI.creature.creatureTemplate, occupyTile.Tile, coord.Tile, maximumDistance * 2, 500, inOpenMedium: true, ref scratchPath);
        return ThreatOfPath(scratchPath, tileCount);
    }

    public virtual float ThreatOfPath(List<IntVector2> path, int pathCount)
    {
        if (path is null ||
            pathCount == 0)
        {
        ...
        {
            threat += ThreatOfTile(AI.creature.Room.realizedRoom.GetWorldCoordinate(path[i]));
            for (int j = 0; j < threatPoints.Count; j++)
            {
                ThreatPoint threatPoint = threatPoints[j];
                if (threatPoint.pos.Tile == path[i])
                {
                    threat += 10f * threatPoint.severity;
                }
            }
        }
        ...
        threat /= pathCount;
        return threat + ThreatOfArea(AI.creature.Room.realizedRoom.GetWorldCoordinate(path[pathCount - 1]));
    }

    public virtual int FindMostAttractiveExit()
    {
        if (aiMap is null)
        {
            return -1;
    ...
    public virtual float ThreatOfArea(WorldCoordinate coord)
    {
        float threat = 0f;
        for (int i = 0; i < 9; i++)
        {
    ...
    public virtual float ThreatOfTile(WorldCoordinate coord)
    {
